...
#' @param group_label The label text for the group.
...
tt_pack_rows <- function(table,
  ...
  if (missing(group_label) || !is.character(group_label) || length(group_label) != 1) {
    rlang::abort("`group_label` must be a single character string")
  }

  if (missing(start_row) || !is.numeric(start_row) || length(start_row) != 1) {
    rlang::abort("`start_row` must be a single number")
  }

  if (missing(end_row) || !is.numeric(end_row) || length(end_row) != 1) {
