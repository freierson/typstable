...
```{r table-4}
tbl <- tt(mtcars[1:5, 1:3], preamble = sans_font, rownames = TRUE) |>
...
```{r table-5}
tbl <- tt(mtcars[1:6, c("mpg", "qsec", "cyl", "disp", "hp", "wt")], preamble = sans_font, rownames = TRUE) |>
  tt_header_above(c(" " = 1, "Performance" = 2, "Characteristics" = 4), line_sep = "5pt") |>
...
tbl <- tt(cars, cols = c(mpg, disp, hp, wt), preamble = sans_font, rownames = TRUE) |>
