...
Default table excludes row names.
...
```{r}
tt(mtcars[1:5, ], cols = c(mpg, cyl, hp))
```
...
```{r}
tt(mt, col_names = c("", "Miles/Gallon", "Cylinders", "Displacement"), rownames = TRUE)
```
...
```{r}
tt(mt, align = c("left", "right", "center"))
```
...
```{r}
tt(mt, col_widths = c("100pt", "2fr", "1fr"))
```
...
```{r}
tt(mt) |>
  tt_style(position = "center")
```

## Full width with fractional units

Achieve full-width layout using fractional units in `tt_widths()`.
...
```{r}
tt(mt10) |>
  tt_pack_rows("Light Label", 1, 5, bold = FALSE)
```

## Styled group labels

Group labels with fill, italic, and hline_below.

```{r}
tt(mt10) |>
  tt_pack_rows("4 Cylinders", 1, 5, fill = "#e6f3ff", italic = TRUE, hline_below = TRUE) |>
  tt_pack_rows("6 Cylinders", 6, 10, fill = "#e6f3ff", italic = TRUE, hline_below = TRUE)
```

## Group label with color and alignment

```{r}
tt(mt10) |>
  tt_pack_rows("Centered Group", 1, 5, align = "center", color = "blue", bold = FALSE)
```
...
## Multiple groups with default line_sep
...
Check that references still work.

```{r}
tt(mt6) |>
  tt_header_above(c("Performance" = 2, "Engine" = 2, "Design" = 2)) |>
  tt_cell(row = 3, column = 3, color = 'red')
```
...
## line_sep=NULL (no separator)

Disable separator between header groups.

```{r}
tt(mt6) |>
  tt_header_above(c("Performance" = 2, "Engine" = 2, "Design" = 2), line_sep = NULL)
```

## line_sep="20pt" (wider separator)

Use wider separator between header groups.

```{r}
tt(mt6) |>
  tt_header_above(c("Performance" = 2, "Engine" = 2, "Design" = 2), line_sep = "20pt")
```
...
```{r}
tt(mt4) |>
  tt_header_above(c("AB" = 2, "CD" = 2), rotate = 45, line_sep = NULL)
```
...
## Two stacked header rows (no separator)

Add two levels of spanning headers without separators.
...
## Three stacked headers with different styling (no separator)
...
tt(mt6, align = 'center') |>
  ...
  tt_header_above(c("Left" = 2, "Right" = 4), fill = "lightgrey") |>
  tt_header_above(c("All" = 6), fill = "navy", color = "white")
```

## Two stacked headers with separator on innermost

Stacked headers where the innermost header has separators.
...
## Three stacked headers with separators

Three stacked levels where the innermost has separators.
...
```{r}
tt(mt6) |>
  tt_header_above(c("A" = 2, "B" = 2, "C" = 2), line_sep = NULL) |>
  tt_header_above(c("Left" = 3, "Right" = 3), line_sep = NULL) |>
...
## Default booktabs=TRUE

Default table with three horizontal rules and no cell borders.

```{r}
tt(mt)
```

## booktabs=FALSE (grid style)

Grid-style table with borders on all cells.

```{r}
tt(mt, booktabs = FALSE)
```

## booktabs=FALSE with header styling

Grid table combined with header row styling.

```{r}
tt(mt, booktabs = FALSE) |>
  tt_row(0, bold = TRUE, fill = "#e0e0e0")
```

## booktabs=FALSE with header_above

Grid table with a spanning header.

```{r}
tt(mt, booktabs = FALSE) |>
  tt_header_above(c("Performance" = 2, "Engine" = 1))
```


# `stroke = FALSE` (Line Suppression)

## Suppress top booktabs rule

Remove the top horizontal rule using `tt_hline(0, stroke = FALSE)`.
...
## Suppress mid booktabs rule

Remove the mid rule (below header) using `tt_row(1, hline_above = FALSE)`.
...
## Suppress bottom booktabs rule

Remove the bottom rule using `tt_hline()` at the last position.

```{r}
tt(mt) |>
  tt_hline(nrow(mt) + 1, stroke = FALSE)
```

## Suppress all three booktabs rules

Remove all default rules for a minimal table.

```{r}
tt(mt) |>
  tt_hline(0, stroke = FALSE) |>
  tt_hline(1, stroke = FALSE) |>
  tt_hline(nrow(mt) + 1, stroke = FALSE)
```

## Suppress mid rule and add custom replacement

Replace the default mid rule with a colored line.

```{r}
tt(mt) |>
  tt_row(1, hline_above = FALSE) |>
  tt_hline(1, stroke = "2pt + blue")
```

## stroke=FALSE on individual cells

Explicitly remove borders from specific cells in a grid table.

```{r}
tt(mt, booktabs = FALSE) |>
  tt_cell(2, 2, stroke = FALSE)
```

## stroke=FALSE on a row in a grid table

Remove borders from an entire row in a grid table.

```{r}
tt(mt, booktabs = FALSE) |>
  tt_row(3, stroke = FALSE)
```

## hline_above=FALSE on a grid table row

Suppress the border above a specific row in a grid table.

```{r}
tt(mt, booktabs = FALSE) |>
  tt_row(3, hline_above = FALSE)
```

## tt_style(stroke=TRUE) overrides booktabs

Setting `tt_style(stroke = TRUE)` on a booktabs table switches to grid and suppresses the default rules.
...
## Override a booktabs rule with custom stroke

Replace the top rule with a thick red line.

```{r}
tt(mt) |>
  tt_hline(0, stroke = "3pt + red")
```

## booktabs=FALSE with vline(stroke=FALSE)

Suppress a vertical line in a grid table.

```{r}
tt(mt, booktabs = FALSE) |>
  tt_vline(1, stroke = FALSE)
```

## styling of tt_pack_rows
...
dat |>
  tt() |>
  tt_pack_rows(index = table(dat$group),  
               hline_below = '0.25pt+gray', italic = TRUE, color = 'orange') |>
  tt_row(0:4) |>
  tt_widths(1,1)
```
