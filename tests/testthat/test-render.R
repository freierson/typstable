...
test_that("tt_render wraps in figure when caption provided", {
  df <- data.frame(a = 1:2, b = 3:4)
  code <- tt(df, caption = "Test table", rownames = FALSE) |> tt_render()
...
test_that("tt_render adds label for cross-reference", {
  df <- data.frame(a = 1:2, b = 3:4)
  code <- tt(df, label = "tbl-test", rownames = FALSE) |> tt_render()
...
test_that("tt_render handles hlines", {
  df <- data.frame(a = 1:2, b = 3:4)
  code <- tt(df, rownames = FALSE) |> tt_style(header_separate = TRUE) |> tt_render()
...
test_that("tt_render handles footnotes", {
  df <- data.frame(a = 1:2, b = 3:4)
  code <- tt(df, rownames = FALSE) |> tt_footnote(general = "Source: test") |> tt_render()
...
test_that("tt_render handles cell background", {
  df <- data.frame(a = 1:2, b = 3:4)
  code <- tt(df, rownames = FALSE) |> tt_cell(1, 1, background = "yellow") |> tt_render()
