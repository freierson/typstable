...
test_that("tt_pack_rows index must be named numeric", {
  ...
  # Unnamed numeric
  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows(index = c(3, 2)),
    "must be a named numeric vector"
  )

  # Named but not numeric
  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows(index = c("A" = "text")),
    "must be a named numeric vector"
  )
})
...
test_that("tt_pack_rows index row count must not exceed table rows", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows(index = c("Group" = 10)),
...
test_that("tt_pack_rows group_label must be single character", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows(group_label = c("A", "B"), start_row = 1, end_row = 3),
    "must be a single character string"
  )

  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows(group_label = 123, start_row = 1, end_row = 3),
    "must be a single character string"
  )
})
...
test_that("tt_pack_rows start_row and end_row must be single numbers", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows("Group", start_row = c(1, 2), end_row = 3),
    "must be a single number"
  )

  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows("Group", start_row = 1, end_row = c(2, 3)),
    "must be a single number"
  )
})
...
test_that("tt_pack_rows row range bounds checking", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows("Group", start_row = 0, end_row = 3),
    "must be between 1 and 5"
  )

  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows("Group", start_row = 6, end_row = 6),
    "must be between 1 and 5"
  )

  # end_row out of range
  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows("Group", start_row = 1, end_row = 0),
    "must be between 1 and 5"
  )

  expect_error(
    tt(df, rownames = FALSE) |> tt_pack_rows("Group", start_row = 1, end_row = 10),
    "must be between 1 and 5"
  )
})
...
test_that("tt_style position warns for invalid values", {
  ...
  expect_warning(
    tt(df, rownames = FALSE) |> tt_style(position = "invalid"),
    "Invalid position"
  )
})
...
test_that("tt_style all valid positions work", {
  ...
  for (pos in c("auto", "left", "center", "right")) {
    tbl <- tt(df, rownames = FALSE) |> tt_style(position = pos)
...
test_that("tt_style full_width = TRUE", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_style(full_width = TRUE)
...
test_that("tt_cell resolves column by character name", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_cell(1, "a", fill = "yellow")
  expect_equal(tbl$cell_styles[["1_1"]]$fill, "yellow")

  tbl2 <- tt(df, rownames = FALSE) |> tt_cell(1, "b", fill = "blue")
...
test_that("tt_cell resolves column by numeric index", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_cell(1, 1, fill = "green")
  expect_equal(tbl$cell_styles[["1_1"]]$fill, "green")

  tbl2 <- tt(df, rownames = FALSE) |> tt_cell(1, 2, fill = "red")
...
test_that("tt_cell errors for invalid column name/index", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_cell(1, "nonexistent", fill = "red"),
    "Column not found"
  )

  expect_error(
    tt(df, rownames = FALSE) |> tt_cell(1, 10, fill = "red"),
    "Column not found"
  )
})
...
test_that("tt_cell rowspan attribute", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_cell(1, 1, rowspan = 2)
...
test_that("tt_cell row range validation with no headers_above", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_cell(0, 1, bold = TRUE)
  expect_true(tbl$cell_styles[["0_1"]]$bold)

  # Negative row should error without headers_above
  expect_error(
    tt(df, rownames = FALSE) |> tt_cell(-1, 1, bold = TRUE),
    "must be between"
  )
})
...
test_that("tt_cell italic attribute", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_cell(1, 1, italic = TRUE)
...
test_that("tt_cell align attribute", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_cell(1, 1, align = "center")
...
test_that("tt_cell row parameter validation", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_cell(c(1, 2), 1, bold = TRUE),
...
test_that("tt_column warns for empty column selection", {
  ...
  expect_warning(
    tt(df, rownames = FALSE) |> tt_column(c(), bold = TRUE),
    "No columns matched"
  )
})
...
test_that("tt_column border_right creates vlines", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_column(a, border_right = "1pt + gray")
...
test_that("tt_column border_left creates vlines", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_column(b, border_left = "2pt + red")
...
test_that("tt_column width parameter overrides default", {
  ...
  tbl <- tt(df, rownames = FALSE) |> tt_column(a, width = "100pt")
...
test_that("tt_column missing column ref warns by default", {
  ...
  expect_warning(
    tt(df, rownames = FALSE) |> tt_column(a, bold = nonexistent),
    "not found"
  )
})
...
test_that("tt_hline warns for out-of-range y", {
  ...
  expect_warning(
    tt(df, rownames = FALSE) |> tt_hline(y = 10),
    "should be between 0 and 4"
  )

  expect_warning(
    tt(df, rownames = FALSE) |> tt_hline(y = -1),
    "should be between 0 and 4"
  )
})
...
test_that("tt_vline warns for out-of-range x", {
  ...
  expect_warning(
    tt(df, rownames = FALSE) |> tt_vline(x = 10),
    "should be between 0 and 2"
  )

  expect_warning(
    tt(df, rownames = FALSE) |> tt_vline(x = -1),
    "should be between 0 and 2"
  )
})
