...
test_that("tt align validation wrong length", {
  ...
  expect_error(
    tt(df, align = c("left", "right"), rownames = FALSE),
...
test_that("tt_widths mixing named/unnamed error", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_widths(a = 1, 2, c = 1),
...
test_that("tt_widths invalid column name error", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_widths(a = 1, nonexistent = 2),
...
test_that("tt_widths requires at least one width", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_widths(),
...
test_that("tt_widths rejects non-positive widths", {
  ...
  expect_error(
    tt(df, rownames = FALSE) |> tt_widths(0, 1),
    "must be positive numbers"
  )

  expect_error(
    tt(df, rownames = FALSE) |> tt_widths(-1, 1),
    "must be positive numbers"
  )
})
